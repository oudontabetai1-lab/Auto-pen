...
class AnthropicProvider(BaseLLMProvider):
    ...
    def __init__(self, model: str = "claude-sonnet-4-6", api_key: str = "", timeout: float = 120.0, temperature: float = 0.1, max_tokens: int = 4096) -> None:
        self.model = model
    ...
    async def chat_with_tools(self, messages: list[LLMMessage], tools: list[dict[str, Any]], system_prompt: str) -> LLMResponse:
        try:
            import anthropic
        ...
        ant_tools = [{"name": t["name"], "description": t.get("description", ""), "input_schema": t.get("parameters", {})} for t in tools]
        ...
        for block in response.content:
            if block.type == "text":
                ...
                tool_calls.append(ToolCall(id=block.id or str(uuid.uuid4()), name=block.name, arguments=block.input if isinstance(block.input, dict) else {}))
        return LLMResponse(content=content_text, tool_calls=tool_calls, model=self.model,
