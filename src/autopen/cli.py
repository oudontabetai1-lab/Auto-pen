...
Exit codes:
  0 — success
  2 — invalid CLI argument / mis-configured provider
  3 — resource not found (session, profile)
  4 — privacy / authorization confirmation declined
"""
...
from autopen.logging_config import configure_logging
...
# Exit-code constants — used consistently across commands.
EXIT_INVALID_ARG = 2
EXIT_NOT_FOUND = 3
EXIT_USER_DECLINED = 4
...
def _validate_max_steps(value: int) -> int:
    if value < 1 or value > 200:
        raise typer.BadParameter("--max-steps must be between 1 and 200")
    return value
...
def _validate_auth_token(value: str) -> str:
    if len(value.strip()) < 20:
        raise typer.BadParameter(
            "--auth-token must be at least 20 characters — paste the full written authorization."
        )
    return value
...
def _check_cloud_privacy(provider: str, accept_cloud: bool) -> None:
    ...
    if not sys.stdin.isatty():
        ...
        raise typer.Exit(EXIT_USER_DECLINED)
...
def scan(
    ...
    auth_token: str = typer.Option(..., "--auth-token", "-a",
                                    help="Written authorization statement (>=20 chars)",
                                    callback=_validate_auth_token),
    ...
    max_steps: int = typer.Option(40, "--max-steps", help="Max agent loop iterations (1-200)",
                                  callback=_validate_max_steps),
    ...
    if offline and provider in _CLOUD_PROVIDERS:
        ...
        raise typer.Exit(EXIT_INVALID_ARG)
    ...
    from autopen.llm.factory import get_provider, LLMConfigError
    ...
    # Validate profile (case-insensitive)
    try:
        scan_profile = ScanProfile(profile.lower())
    except ValueError:
        console.print(
            f"[red]Invalid profile '{profile}'.[/red] "
            "Choose: web, network, cloud, ctf (case-insensitive)."
        )
        raise typer.Exit(EXIT_INVALID_ARG)
...
def sessions_show(
    ...
    from autopen.state.manager import mask_authorization_token
    ...
    console.print(f"  Auth:      {mask_authorization_token(session.authorization_token)}")
...
def report(
    ...
    if format not in {"markdown", "json"}:
        console.print(f"[red]Unsupported format '{format}'. Use markdown or json.[/red]")
        raise typer.Exit(EXIT_INVALID_ARG)
